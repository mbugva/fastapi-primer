...
async def test_create_user_duplicate_email(client: AsyncClient):
    ...
    assert response.json()["detail"] == "Username or email already exists. Try again?"
...
async def test_upload_profile_picture(client: AsyncClient, mocked_aws):
    ...
    test_image_path = Path(__file__).parent.parent.parent / "test_image.jpg"
...
async def test_forgot_password_sends_email(client: AsyncClient):
    ...
    # temporarily replace real "send_password_reset_email" function locally
    # in the auth namespace in which it is used, and not the EmailUtils class
    # from which it is created.
    # NOTE: This is a common pitfall for mocking with unitest. Always mock
    # where the function is looked up, not where it is defined
    with patch(
        "app.routers.api_v1.auth.send_password_reset_email",
