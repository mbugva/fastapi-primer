...
async def test_get_post_not_found(client: AsyncClient):
    # NOTE: even though your database could be populates, each test function
    # begins with a fresh instance of the db, hence always empty
    response = await client.get("/api/posts/1")
    assert response.status_code == 404

    data = response.json()
    data["detail"] == ["Oops! Looks like the post does not exist. Try again?"]
...
async def test_create_post_success(client: AsyncClient):
    ...
    response = await client.post(
        ...
        json={
            "title": "My first post",
            "content": "Testing isnt that bad after all ;/",
        },
        headers=headers,
    )
    ...
    assert data["content"] == "Testing isnt that bad after all ;/"
...
async def test_update_post_wrong_user(client: AsyncClient):
    ...
    token1 = await login_user(client, username="user1")
    ...
    token2 = await login_user(client, username="user2")
    ...
    assert (
        ...
        == "Oops! Looks like you are not authorized to update this post."
    )
